...
import polyphonic_dataloader as poly
from util import seq_collate_fn
import pytorch_lightning as pl
from pytorch_lightning import loggers as pl_loggers

class DeepMarkovModel(pl.LightningModule):
    ...
    def __init__(self, config):
        ...
        self.transition = Transition(self.z_dim, self.trans_dim,
                                     gated=True, identity_init=True)
        # inference model
        self.combiner = Combiner(self.z_dim, self.rnn_dim)
        self.encoder = RnnEncoder(self.input_dim, self.rnn_dim,
                                  n_layer=1, drop_rate=0.0,
                                  bd=False, nonlin='relu')

        # initialize hidden states
        self.z_0 = self.transition.init_z_0()
    ...
    def forward(self, x, x_reversed, x_mask, x_seq_lengths):
        ...
        # include the kl loss at the 0-th time-step (?)
